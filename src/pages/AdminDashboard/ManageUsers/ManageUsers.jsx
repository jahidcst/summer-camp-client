...
const ManageUsers = () => {
  ...
  const handleMakeAdmin = (user) => {
    axiosSecure.patch(`https://summer-vacetion-server-jahidcst.vercel.app/users/admin/${user._id}`)
      .then((data) => {
  ...
  const handleMakeInstructor = (user) => {
    axiosSecure.patch(`https://summer-vacetion-server-jahidcst.vercel.app/users/instructor/${user._id}`)
      .then((data) => {
        if (data.data.modifiedCount > 0) {
          refetch();
          Swal.fire({
